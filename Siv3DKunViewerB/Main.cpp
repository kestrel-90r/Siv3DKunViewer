...
Array <PrecomputeAnime> precAnimes;
...
void gltfSetupModel( tinygltf::Model& model,int anime )
{
...
#if 1
    const auto aid = anime;                      // 現状アニメは１つのみ
    auto& man = model.animations[aid];

    int cycleframe = 170;                     // ループアニメ1周のフレーム数(60フレーム)
    auto cycletime = 2.8f;                   // ループアニメ周期1秒
    ...
    for (auto aa = 0; aa < cycleframe; aa++)
    {
        ...
        for (auto& ch : precAnimes[aid].Frames[aa].Channels)
        {
            ...
            size_t kf;
            for (kf=0 ; kf < sa.interpolateKeyframes.size()-1; kf++)    //219のサンプラからいちいち１つの該当キーフレーム選択するの無駄
            {
        ...
        for (int nn = 0; nn < model.scenes[0].nodes.size(); nn++)
        {
            ...
            for (int cc = 0; cc < msn.children.size(); cc++)
            {
                ...
                if (node.mesh >=0 )//メッシュノード　
                {
                    ...
                    gltfPrecomputeMesh(aa, precAnimes[anime], node);
                }
            }
        }
...
void gltfPrecomputeMesh(int idxframe, PrecomputeAnime& precanime, tinygltf::Node& node )
{
...
// #define STOPMOTION       //スプラインバグ調査用コマ送り確認
...
#ifndef STOPMOTION
    if (++cfr >= anime.Frames.size()-2) cfr = 0;    //スプラインバグ隠蔽工作-2
#endif
}
...
void Main()
{
    ...
    gltfSetupModel( model , 2);     //アニメ２を選択
    ...
    while (System::Update())
    {
        ...
        gltfDrawPrecomputeAnime( precAnimes[2], Float3(0, 0, 0), Float3(1, 1, 1), Quaternion(0, 0, 0, 1));
